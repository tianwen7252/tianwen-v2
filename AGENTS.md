...
- `pnpm dev` for vite, `pnpm run api` for vercel API.
...
See `.codex/docs/jira-workflow.md` for the Jira integration workflow. Read it before running epic-plan or any Jira-related operations.
...
When the user asks for a **pr release**, execute the complete release workflow:

1. Commit all intended changes on a feature branch with a conventional commit.
2. Push the branch and open a pull request targeting `main`.
3. Track required CI checks and merge the pull request when they pass.
4. Track the Release Please pull request created from `main`.
5. Verify that the Release Please pull request contains the requested stable version.
6. Merge the Release Please pull request after its required checks pass.
7. Confirm the matching GitHub Release and tag were published successfully.

Do not report a pr release as complete until the final GitHub Release exists.
