...
3. **Discounts Applied**: Manual discounts added to cart
4. **Submit Initiated**: submitOrder() called with cart state
5. **Order Created**: DB rows inserted (order, order_items, order_discounts)
6. **Daily Data Updated**: Aggregated totals for analytics
7. **Cart Cleared**: useOrderStore reset for next order
