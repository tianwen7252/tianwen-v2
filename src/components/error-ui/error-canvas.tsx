...
// ─── Shader Sources ─────────────────────────────────────────────────────────
// Copied verbatim from error.html — Schwarzschild black hole ray tracer
...
/* eslint-disable */
// @ts-ignore — raw GLSL string, not TypeScript
...
/**
 * WebGL Event Horizon animation — ported 1:1 from error.html.
 * Uses the same setup pattern as InitCanvas (ref + useEffect + RAF).
 */
export function ErrorCanvas({ className }: ErrorCanvasProps) {
  ...
  useEffect(() => {
    ...
    // ── WebGL init (mirrors error.html exactly) ──
    ...
    function compile(type: number, src: string): WebGLShader | null {
      ...
      if (!gl!.getShaderParameter(s, gl!.COMPILE_STATUS)) {
        logError(`Shader compile: ${gl!.getShaderInfoLog(s)}`, 'ErrorCanvas')
        return null
      }
      return s
    }
    ...
    if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
      logError(`Program link: ${gl.getProgramInfoLog(prog)}`, 'ErrorCanvas')
      return
    }
    gl.useProgram(prog)

    // Full-screen triangle (same as error.html)
    ...
    // Default values (same as error.html)
    ...
    // ── Animation loop (mirrors error.html + InitCanvas pattern) ──
    let rafId = 0
    let running = true
